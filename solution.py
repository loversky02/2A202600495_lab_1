...
def compare_models(prompt: str) -> dict:
    ...
    # Calculate cost estimate for GPT-4o
    # 0.75 words ≈ 1 token approximation
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        history = history[-6:]  # Keep last 3 turns (3 user + 3 assistant = 6 messages)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    table = []
    table.append("=" * 150)
    table.append(f"{'Prompt':<42} | {'GPT-4o Response':<42} | {'Mini Response':<42} | {'GPT-4o Lat':<12} | {'Mini Lat':<12}")
    table.append("=" * 150)
    
    # Build table rows
    for result in results:
        prompt = truncate(result.get("prompt", ""), 40)
        gpt4o_resp = truncate(result.get("gpt4o_response", ""), 40)
        mini_resp = truncate(result.get("mini_response", ""), 40)
        gpt4o_lat = f"{result.get('gpt4o_latency', 0):.3f}s"
        mini_lat = f"{result.get('mini_latency', 0):.3f}s"
        
        table.append(f"{prompt:<42} | {gpt4o_resp:<42} | {mini_resp:<42} | {gpt4o_lat:<12} | {mini_lat:<12}")
